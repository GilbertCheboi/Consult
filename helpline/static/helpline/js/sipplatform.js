...
var configOptions = {
    ...
    sipProxy    : 'sip:sip-proxy.callcenter.africa:5868',
};
								...
								onmessage: function(msg, jsep) {
									...
									if(result && result["event"]) {
										...
										if(event === 'registered') {
											...
											showNotification("Incoming Call", "Incoming call from " + caller_uri.user + "!");
                                            ...
                                            if($('#toggle_auto_answer_btn').html() === "On")
                                            {
                                                ...
                                                incoming = bootbox.dialog({
                                                    message: "Incoming call from " + caller_uri.user + "!" + transfer + rtpType + extra,
                                                    title: "Incoming call",
                                                    closeButton: false,
                                                    buttons: {
                                                        success: {
...
function doCall(ev) {
	...
	$('#transactionid' + suffix).attr('disabled', true);
    $( ".btn-digit" ).addClass( "btn-dtmf" );
	$('#call' + suffix).attr('disabled', true).unbind('click');
	$('#dovideo' + suffix).attr('disabled', true);
	var username = $('#peer' + suffix).val().trim();
	var transactionid = $('#transactionid' + suffix).val().trim();
	var sid = $('#sid' + suffix).val().trim();
	var channelId = $('#channelId' + suffix).val().trim();
    ...
    localStorage.lastTransactionId = transactionid;
    playBeep();
	actuallyDoCall(handle, username, doVideo, null,transactionid, sid, channelId);
}
function actuallyDoCall(handle, uri, doVideo, referId, transactionId, sid, channelId) {
	handle.createOffer(
		{
			media: {
			...
			success: function(jsep) {
				...
				var body = { 
                    ...
                    uri: uri,
                    headers: {
                        "X-TransactionID": transactionId, // Add Transaction ID processed by PBX
                        "X-PlatformVersion": "V2",
                        "X-SID": sid,
			...
			onmessage: function(msg, jsep) {
				...
				if(result && result["event"]) {
					...
					if(event === 'registered') {
                        ...
                        playBeep();
						caller_uri = parseUri(result["username"]);
						Janus.log("[Helper #" + helperId + "] Incoming call from " + caller_uri.user + "! (on helper #" + helperId + ")");
						...
						if(offerlessInvite)
							extra = " (no SDP offer provided)"
						incoming = bootbox.dialog({
							message: "Incoming call from " + caller_uri.user + "!" + transfer + rtpType + extra + " (on helper #" + helperId + ")",
