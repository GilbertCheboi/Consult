...
function registerSIPdetails(data) {
    ...
    if (data.turn_server_uri) {
      var turn_server_uri = data.turn_server_uri;
      var turn_username = data.turn_server_username;
      var turn_password = data.turn_server_password;
      var iceServers = [{"urls":turn_server_uri, "username":turn_username, "credential":turn_password}];
      console.log("USING ICE SERVER " + turn_server_uri);
    } else {
      var turn_server_uri = "stun:stun.l.google.com:19302";
      var turn_username = null;
      var turn_password = null;
      var iceServers = [{urls: turn_server_uri}];
    }
...
function getSIPdetails(){
    ...
    $.ajax({
        ...
        success : function(data){
            configOptions.hotDesks = data.hotdesk_data.slice();
            primary_account = null;
            data.hotdesk_data.forEach(addHotdeskLink);
